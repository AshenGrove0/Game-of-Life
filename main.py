...
alive_coords_to_start = [(3,2),(2,1)]
def main():
    board = generate_starting_board()
    pprint.pp(board)
    run(board,0,10) # let user choose num2
    #do this recursively
...
def run(board: list, num_recursions_so_far,num_recursions_wanted):
    ...
    if not all_dead:
        run(new_board,num_recursions_so_far, num_recursions_wanted)
